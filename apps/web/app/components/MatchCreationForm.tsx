...
export function MatchCreationForm({ requestedDiscipline }: { requestedDiscipline: string }) {
  ...
  return (
    <form className="entity-form" onSubmit={(event) => void submit(event)} noValidate>
      ...
      <label><span>Categoria / chave do InterEng</span><select value={effectiveTournamentId} onChange={(event) => { setTournamentId(event.target.value); setTeamA(''); setTeamB(''); setError(''); }} required disabled={!discipline || !disciplineTournaments.length}><option value="" disabled>Selecione a categoria</option>{disciplineTournaments.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label>
      ...
      {phaseOptions.length > 1 ? <label><span>Fase / grupo do confronto</span><select value={effectivePhase} onChange={(event) => { setPhase(event.target.value); setError(''); }} disabled={!tournament}>{phaseOptions.map((item) => <option key={item}>{item}</option>)}</select></label> : null}
