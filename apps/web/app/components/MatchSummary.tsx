...
import { getMatchStatusLabel, isLive, matchStatus } from '@atletica-incinera/intereng-contract/rules';
...
export function MatchSummary({ match }: MatchSummaryProps) {
  const tone = isLive(match.status) ? 'pink' : match.status === matchStatus.finished ? 'neutral' : 'orange';
