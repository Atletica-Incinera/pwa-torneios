...
export function TeamAthleteForm({ teamId }: { teamId: string }) {
  ...
  // O gestor cadastra atleta apenas na propria modalidade — e o servidor
  // recusa qualquer outra. Mostrar a lista inteira aqui seria oferecer uma
  // escolha que termina em erro depois de preencher o formulario todo.
  const managerScope = session?.role === 'DISCIPLINE_MANAGER' ? session.scope : undefined;
  const disciplines = listDisciplines(state, getActiveEdition(state)?.id)
    .filter((item) => item.enabled && (!managerScope || item.name === managerScope))
    .map((item) => item.name);
  const [step, setStep] = useState<1 | 2>(1); const [name, setName] = useState(''); const [selectedDisciplines, setSelectedDisciplines] = useState<string[]>(managerScope ? [managerScope] : []); const [error, setError] = useState(''); const [submitting, setSubmitting] = useState(false);
