...
export function DisciplineSelector({ options }: DisciplineSelectorProps) {
  ...
  const selected = options.includes(requested) ? requested : (options[0] ?? '');
  ...
  return (
    <div className="discipline-selector" role="group" aria-label="Filtrar por modalidade">
      ...
      <div className="discipline-selector-track">
        {options.map((option) => (
          <button type="button" className={`discipline-option${selected === option ? ' active' : ''}`} onClick={() => handleChange(option)} key={option}>
            {option}
          </button>
        ))}
      </div>
