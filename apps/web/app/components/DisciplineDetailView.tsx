...
export function DisciplineDetailView({ id, initial, index }: { id: string; initial?: DisciplineBase; index: number }) {
  ...
  return <AppShell active="profile" eyebrow="MODALIDADE" title={discipline.name.toUpperCase()} subtitle="Configuração da edição 2026 do InterEng"><section className="discipline-hero"><span>{String(index + 1).padStart(2, '0')}</span><div><StatusBadge tone={discipline.tone}>{discipline.mode}</StatusBadge><h2>{discipline.name}</h2><p>{discipline.tournaments} categorias nesta edição</p></div></section><section className="section-block"><SectionTitle eyebrow="REGRAS" title="CONFIGURAÇÃO" /><DisciplineManager name={discipline.name} mode={discipline.mode} initialConfig={discipline.config} tournaments={discipline.tournaments} /></section></AppShell>;
}
