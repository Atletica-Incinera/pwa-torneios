...
export function TeamCreationForm() {
  ...
  return <AppShell active="teams" eyebrow="CATÁLOGO GLOBAL" title="NOVA EQUIPE" subtitle="Cadastre a equipe uma única vez"><form className="entity-form" onSubmit={submit} noValidate><label><span>Nome da equipe</span><input value={name} onChange={(event) => { setName(event.target.value); setError(''); }} placeholder="Ex.: Alcateia" autoFocus required /></label><label><span>Sigla</span><input value={initials} onChange={(event) => { setInitials(event.target.value); setError(''); }} placeholder="ALC" minLength={2} maxLength={8} required /></label><label><span>Responsável</span><input value={responsible} onChange={(event) => { setResponsible(event.target.value); setError(''); }} placeholder="Nome do responsável" required /></label>{error ? <p className="form-feedback form-feedback-error" role="alert">{error}</p> : null}<div className="form-actions"><Link href="/teams" className="secondary-button">Cancelar</Link><button type="submit" className="primary-button" disabled={submitting}>{submitting ? 'Cadastrando…' : 'Cadastrar equipe'}</button></div></form></AppShell>;
}
