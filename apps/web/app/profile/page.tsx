...
export default function ProfilePage() {
  const router = useRouter(); const [open, setOpen] = useState(false); const { session, logout: clearSession } = useFrontendSession(); const { state, commit } = useFrontendState(); const { toast } = useUi(); const notifications = state.preferences.notifications;
  async function updateNotifications(value: boolean) { let enabled = value; if (value) { if (!('Notification' in window)) { toast('Este navegador não oferece notificações push.', 'error'); enabled = false; } else { const permission = await Notification.requestPermission(); enabled = permission === 'granted'; if (!enabled) toast('Permissão de notificações não concedida.', 'error'); } } commit((current) => ({ ...current, preferences: { ...current.preferences, notifications: enabled } }), { action: 'Preferência de notificações alterada', entity: session?.name ?? 'Perfil', after: enabled ? 'Ativadas' : 'Desativadas' }); }
