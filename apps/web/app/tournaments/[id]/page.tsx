...
export default async function TournamentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AppShell active="tournaments" eyebrow={`${tournament.discipline.toUpperCase()} • TORNEIO`} title={tournament.name.toUpperCase()} subtitle={`${tournament.phase} • InterEng 2026`}>
      ...
      <section className="section-block" id="results"><SectionTitle eyebrow="DESEMPENHO" title="CLASSIFICAÇÃO E CHAVEAMENTO" /><TournamentResults tournamentId={tournament.id} discipline={tournament.discipline} fallbackParticipants={standings.map((entry) => entry.name)} /></section>
    </AppShell>
  );
}
